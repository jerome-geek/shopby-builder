...
const swiperOptionsSchema = z.object({
    ...
    slidesPerView: z.number().min(1).max(10).default(6).optional(),
});

const iconBannerSchema = z.object({
    swiperOptions: swiperOptionsSchema.optional(),
});

type IconBannerFormData = z.infer<typeof iconBannerSchema>;

export const IconBannerProperties: React.FC<{
    blockId: string;
    initialData?: Partial<IconBannerFormData>;
}> = ({ blockId, initialData }) => {
    ...
    const {
        ...
        reset,
        formState: { isDirty },
    } = useForm<IconBannerFormData>({
        resolver: zodResolver(iconBannerSchema),
        defaultValues: {
            swiperOptions: {
                loop: false,
                pagination: true,
    ...
    const onSubmit = (data: IconBannerFormData) => {
